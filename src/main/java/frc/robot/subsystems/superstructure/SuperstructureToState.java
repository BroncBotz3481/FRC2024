...
public class SuperstructureToState extends SequentialCommandGroup {
    ...
    public SuperstructureToState(Superstructure superstructure,SuperState targetState){
        ...
        Command intakeCmd = Commands.waitUntil(m_intakeWait).andThen(superstructure.m_intake.positionIntake(m_targetState.intake.position).alongWith(superstructure.m_intake.runIntake(m_targetState.intake.power)).until(m_intakeUntil));
